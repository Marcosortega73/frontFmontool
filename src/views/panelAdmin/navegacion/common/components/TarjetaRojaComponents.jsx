...
const TarjetaRojaComponents = ({partido,torneo, visitante, setSearchVisitante,local,setSearchLocal,setSelectedVisitante,setSelectedLocal }) => {
  ...
  const [rojaLocal, setRojaLocal] = React.useState([]);
  ...
  const handleDeleteChipLocal = (jugador) => {
    console.info("You clicked the delete icon.");
    console.log("ID", jugador?.id);
    const goleadores = rojaLocal.filter((goleador) => {
      return goleador.id !== jugador?.id;
    });

    setRojaLocal(goleadores);
        ...
        <Grid item xs={12} md={6} lg={6}>
          ...
          {rojaVisitante?.length > 0 ? (
            ...
            >
              ...
              {rojaVisitante?.map((jugador, index) => {
                //cantidad de gole
                return (
                  <>
                    <ListItem
